...
def convert_dim_to_base_cgpm(dim, rng):
...
def convert_cgpm_state_to_cgpm2(state, rng):
    views = [convert_view_to_rowmixture(v, rng) for v in state.views.values()]
    return Product(views, rng=rng)
